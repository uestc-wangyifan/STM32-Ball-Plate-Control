#include "oled.h"
#include "codetab.h"//oled字库
#include "main.h"   // 引入工程管家
#include "stm32f1xx_hal.h"  // 【核心修复】直接暴力引入 HAL 库核心大脑，让它认识 HAL_Delay！
//功能：显示一个或多位数字
//wgn 2021.4.29
...
// Description    : 显示codetab.h中的ASCII字符,有6*8和8*16共两款可选
//--------------------------------------------------------------
...
// Calls          : 
// Parameters     : x0,y0 -- 起始点坐标(x0:0~127, y0:0~7); x1,y1 -- 对角线(右下角)坐标(x1:1~128,y1:1~8)
// Description    : 显示BMP位图
//--------------------------------------------------------------
...
void OLED_Init(void)
{
	...
	OLED_Command(0xc8);	//Set COM Output Scan Direction (c8，正常是c0)
	OLED_Command(0xa1); //--set segment re-map 0 to 127 (a1，正常是a0)
